                    ...
                    using (XmlReader reader = XmlReader.Create(fileName, xrset))
                    {
                        while (reader.Read())
                        {
                            if (reader.NodeType == XmlNodeType.Element
                                && reader.Name == "NPC")
                            {
                                int SMapId = int.Parse(reader.GetAttribute(0));
                                if (!Spawns.ContainsKey(SMapId))
                                    Spawns.Add(SMapId, new List<SpawnTemplate>());

                                Spawns[SMapId].Add(new SpawnTemplate
                                {
